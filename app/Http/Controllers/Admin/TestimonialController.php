...
class TestimonialController extends Controller
{
    ...
    public function index()
    {
        ...
        //SEOMeta::setTitle('Dashboard - Testimonials - ' . (empty($settings->setting_site_name) ? config('app.name', 'Laravel') : $settings->setting_site_name));
    ...
    public function create()
    {
         ...
         */
        $settings = Setting::find(1);
        //SEOMeta::setTitle('Dashboard - Create Testimonial - ' . (empty($settings->setting_site_name) ? config('app.name', 'Laravel') : $settings->setting_site_name));
    ...
    public function store(Request $request)
    {
        ...
        if(!empty($testimonial_image)){
            ...
            $testimonial_image_name = str_slug($testimonial_name).'-'.$currentDate.'-'.uniqid().'.png';

            if(!Storage::disk('public')->exists('testimonial')){
                Storage::disk('public')->makeDirectory('testimonial');
            }

            $new_testimonial_image = Image::make(base64_decode(preg_replace('#^data:image/\w+;base64,#i', '',$testimonial_image)))->stream();
    ...
    public function edit(Testimonial $testimonial)
    {
         ...
         */
        $settings = Setting::find(1);
        //SEOMeta::setTitle('Dashboard - Edit Testimonial - ' . (empty($settings->setting_site_name) ? config('app.name', 'Laravel') : $settings->setting_site_name));
