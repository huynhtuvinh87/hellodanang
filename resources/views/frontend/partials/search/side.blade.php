    ...
    <div class="mb-2">
            ...
            <div class="select-wrap">
                ...
                <select class="form-control rounded @error('categories') is-invalid @enderror" name="categories">
                    ...
                    @foreach($search_all_categories as $category)
                        <option {{ $category->id == old('categories') ? 'selected' : '' }} value="{{ $category->id }}">{{ $category->category_name }}</option>
                    @endforeach
                </select>
                @error('categories')
        ...
        <div class="form-group">
            ...
            <div id="multiple-datasets" class="wrap-icon">
                ...
                <input name="city_state" type="text" class="form-control rounded typeahead @error('city_state') is-invalid @enderror" value="{{ old('city_state') }}" placeholder="{{ __('frontend.search.state-or-city') }}" aria-describedby="basic-addon1">
                @error('city_state')
